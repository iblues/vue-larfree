  ...
  <div class="app-container">
    <ui-plane>
      ...
      <el-row v-loading="loading" :gutter="20">
        ...
        <el-col v-for="(config,key) in configs" :key="config.key" :xs="24" :sm="12" :md="8" :lg="6">
          <el-card class="box-card">
            ...
            <div class="box-content">
              <div v-for="(field,key) in config" :key="key" class="text item">
                {{ field.name }}
                <router-link :to="`/dialog/edit/system.config/`+config.key+'/'+config.key">
                  <span class="right_item">编辑</span>
                </router-link>
              </div>
            </div>
...
export default {
  ...
  computed: {
    ...
    dataRefreshEvents() {
      return this.getRefreshEvents('schemas')
    }
  },
  watch: {
  ...
  methods: {
    getList() {
      this.loading = true
      this.$http.get('/system/schema').then((res) => {
        console.log(res)
